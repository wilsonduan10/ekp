...
mkpath(joinpath(@__DIR__, "../images/uw_observable"))
cfsite = 23
month = "01"
...
for x in uw_data
    global variance
    variance += (mean(uw_data) - x) * (mean(uw_data) - x)
end
variance /= T
...
title!("uw comparison (ρθq)")
xlabel!("T")
ylabel!("uw")
png("images/uw_observable/y_vs_model1")

theta_true = (4.7, 4.7, 15.0, 9.0)
model_truth = physical_model(theta_true, inputs, true)
plot(y, label="y")
plot!(model_truth, label="Model Truth")
title!("uw comparison (pTq)")
xlabel!("T")
ylabel!("uw")
png("images/uw_observable/y_vs_model2")
