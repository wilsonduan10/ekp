...
outputdir = "images/linear_transformation/bc_$(cfSite)_$(month)_0"
...
parameterTypes = (:b_m, :b_h, :slope, :offset)
...
function G(parameters)
    ...
    return H(Ψ) * parameters[3] .+ parameters[4]
end
...
prior_u3 = constrained_gaussian("slope", 1, 0.3, 0, 10)
prior_u4 = constrained_gaussian("offset", 0.1, 0.3, -1, 1)
...
println("Mean slope:", mean(constrained_initial_ensemble[3, :]))
println("Mean offset:", mean(constrained_initial_ensemble[4, :]))
println()

println("FINAL ENSEMBLE STATISTICS")
println("Mean b_m:", mean(final_ensemble[1, :]))
println("Mean b_h:", mean(final_ensemble[2, :]))
println("Mean slope:", mean(final_ensemble[3, :]))
println("Mean offset:", mean(final_ensemble[4, :]))

theta_true = (15.0, 9.0, 1, 0)
model_truth = G(theta_true)
plot(model_truth, label="Model Truth", c=:red, seriestype=:scatter, ms=5)
plot!(y, label="y", c=:green, seriestype=:scatter, ms=5, markerstroke="green", markershape=:utriangle)
xlabel!("Time")
