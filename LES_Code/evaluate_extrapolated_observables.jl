...
function generate_plot(y, model_truth, name)
    plot(y, label="y")
    plot!(model_truth, label="Model Truth")
    xlabel!("T")
    ylabel!(name)
    title!("$(name) Comparison (pTq)")
    png("images/LES_observables_extrapolated/observables_$(cfsite)_$(month)/$(name)_pTq")
end

mkpath("images/LES_observables_extrapolated")
mkpath("images/LES_observables_extrapolated/observables_$(cfsite)_$(month)")

generate_plot(u_star_data, u_star_model, "u_star")
generate_plot(shf_data, shf_model, "shf")
generate_plot(lhf_data, lhf_model, "lhf")
generate_plot(buoyancy_flux_data, buoy_model, "buoyancy_flux")
generate_plot(τ_data, tau_model, "τ")
generate_plot(vec(mean(qt_data, dims=1)), evaporation_model, "evaporation")
println("Plots generated in folder: images/LES_observables_extrapolated/observables_$(cfsite)_$(month)")
