...
ρ_data = nothing
θ_li_data = nothing
sfc_input = (; ρ_data, p_data, surface_temp_data=T_sfc_data, temp_data=T_data, θ_li_data, qt_data=q_data)
...
function physical_model(parameters, inputs)
    ...
    for j in 1:T # 865
        u_star_sum = 0.0
        total = 0
        ts_sfc = TD.PhaseEquil_ρTq(thermo_params, ρ_sfc_data, T_sfc_data[j], qt_sfc_data[j]) # use 1 to get surface conditions
