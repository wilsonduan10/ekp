...
function plot_y_versus_model(x, y, model, theta_true, inputs, kwargs)
    ...
    plot(x, y, c = :green, label = "y", legend = :bottomright, ms = 8, seriestype=:scatter, markerstroke="green", markershape=:utriangle)
    # plot!(x, truth, c = :red, label = "Model Truth", legend = :bottomright, linewidth = 2, markerstrokecolor="red")
    plot!(x, truth, c = :red, label = "Model Truth", legend = :bottomright, ms = 5, seriestype=:scatter, markerstrokecolor="red")
    if (!isnothing(axes))
        xlabel!(axes[1])
        ylabel!(axes[2])
    end
...
function plot_all(x, y, model, theta_true, inputs, ensembles, N_ensemble, kwargs)
    ...
    plot!(x, initial, c = :red, label = initial_label, linewidth = 2)
    plot!(x, final, c = :blue, label = final_label, linewidth = 2)
    if (!isnothing(axes))
        xlabel!(axes[1])
        ylabel!(axes[2])
    end
